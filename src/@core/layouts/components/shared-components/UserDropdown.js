...
const UserDropdown = () => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      const storedUser = localStorage.getItem('googleAccount')
      if (storedUser) {
  ...
  if (user)
    return (
      <Fragment>
        ...
        >
          <Avatar alt='John Doe' onClick={handleDropdownOpen} sx={{ width: 40, height: 40 }} src={user.photoURL} />
        </Badge>
        <Menu
          anchorEl={anchorEl}
        ...
        */}
          ...
          <Divider />
          <MenuItem sx={{ py: 2 }} onClick={() => logoutApp()}>
    ...
    return (
      <Fragment>
        <Badge
          ...
          anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }}
        >
          <Avatar onClick={handleDropdownOpen} sx={{ width: 40, height: 40 }} src='/images/avatars/1.png' />
        </Badge>
        <Menu
          anchorEl={anchorEl}
          open={Boolean(anchorEl)}
